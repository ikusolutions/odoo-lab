...
@app.command(name="module-install")
def module_install(
    db_arg: str | None = typer.Argument(None, help="Nombre de la base de datos"),
    modules_arg: str | None = typer.Argument(None, help="Módulos coma-separados"),
    db: str | None = typer.Option(
        None, "--db", "-d", help="Nombre de la base de datos"
    ),
    modules: str | None = typer.Option(
        None, "--modules", "-m", help="Módulos coma-separados"
    ),
    timeout: int = typer.Option(
        1800, "--timeout", "-t", help="Timeout en segundos (default 1800)"
    ),
):
    """Instalar módulos en una DB. Uso: module-install DB sale,purchase. Atajos: -d/--db, -m/--modules, -t/--timeout."""
    db = db or db_arg
    ...
    console.print(
        f"\n  [heading]Instalando {', '.join(mods)} en DB '{ctx.db}'...[/heading]\n"
    )

    try:
        rc, _captured = stream_subprocess(
            cmd,
            f"Instalando {', '.join(mods)}",
    ...
    if rc == 0:
        console.print(f"\n  {OK} Módulos instalados: {', '.join(mods)}\n")
    else:
